...
mod runtime_contracts {
    ...
    fn test_initial_state_and_terminal_edges() {
        assert!(matches!(
            Machine::builder().state_count(3).event_count(3).build(),
            Err(FastStateMachineBuildError::InitialStateNotConfigured)
        ));
        for target in [0, 1] {
            assert!(
                matches!(Machine::builder().state_count(3).event_count(3).initial_state(0).terminal_state(0)
                .transition(0, 0, target).build(),
                Err(FastStateMachineBuildError::TerminalStateHasOutgoingTransition { state: 0, event: 0, target: actual }) if actual == target)
            );
        }
    ...
    fn test_self_transition_commits_once_and_failure_skips_callback() {
        ...
        assert!(matches!(
            Machine::builder()
                ...
                .transition(0, 0, 2)
                .build(),
            Err(FastStateMachineBuildError::DuplicateTransition { .. })
        ));
    }
